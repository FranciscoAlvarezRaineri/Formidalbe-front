...
export const DEFAULT_INPUT_NAME = "Nuevo Elemento ";

export function getIdFromElementsBlock(elements) {
...
// generate an array of Card and Section components from a sche
  ...
  const elementList = elementPropArr.map((elementProp, index) => {
    ...
    if (elementProp.propType === "card") {
      ...
      return createElement(
        "div",
        null,
        createElement("h2", null, " Error parsing element ")
      );
    }
  });
  return elementList;
...
export function onDragEnd(result, details) {
  ...
  const dest = result.destination?.index || result.source.index;
  const newElementObjArr = generateElementPropsFromSchemas({
    schema,
    uischema,
    definitionData,
...
function propagateElementChange(
  ...
  elementArr.forEach(function (element) {
    ...
    if (element.propType === "section") {
      ...
      var updatedChildren = propagateElementChange(
        childrenElements,
        definitionData,
        definitionUi,
        categoryHash
      );
      var newUiSchema = Object.assign(
        { ...element.uischema },
...
export function propagateDefinitionChanges(
  ...
  var updatedBodyElements = propagateElementChange(
    bodyElements,
    definitionData,
    definitionUi,
    categoryHash
